...
#include <vector>
#include <string>
#define LABEL_NALE_TXT_PATH "/etc/coco_80_labels_list.txt"
...
static int loadLabelName(const char *locationFilename, char *label[])
{
    ...
    if (ret <= 0) {
        // 尝试上一级目录，兼容在 build 目录下运行的情况
        std::string fallback = "../";
        fallback += locationFilename;
        ret = readLines(fallback.c_str(), label, OBJ_CLASS_NUM);
    }
    return (ret > 0) ? 0 : -1;
}
