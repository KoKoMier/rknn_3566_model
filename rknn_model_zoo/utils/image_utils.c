...
static int crop_and_scale_image_yuv420sp(unsigned char *src, int src_width, int src_height,
    ...
    crop_and_scale_image_c(2, src_uv, src_width / 2, src_height / 2, crop_x / 2, crop_y / 2, crop_width / 2, crop_height / 2,
        dst_uv, dst_width / 2, dst_height / 2, dst_box_x / 2, dst_box_y / 2, dst_box_width / 2, dst_box_height / 2);

    return 0;
}

static void yuv420sp_to_rgb888(unsigned char* yuv_src, int src_width, int src_height, int format, 
                               unsigned char* rgb_dst, int dst_width, int dst_height, 
                               int x_offset, int y_offset, int width, int height)
{
    int y, x;
    unsigned char *y_ptr = yuv_src;
    unsigned char *uv_ptr = yuv_src + src_width * src_height;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            if (cur_y < 0 || cur_y >= src_height || cur_x < 0 || cur_x >= src_width) continue;

            int Y = y_ptr[cur_y * src_width + cur_x];
            int U_idx = (cur_y / 2) * src_width + (cur_x & ~1);
            int V_idx = U_idx + 1;
            int U, V;
            if (format == IMAGE_FORMAT_YUV420SP_NV12) {
                U = uv_ptr[U_idx];
                V = uv_ptr[V_idx];
            } else {
                V = uv_ptr[U_idx];
            ...
            int out_x = x_offset + x;
            int out_y = y_offset + y;
            if (out_x < 0 || out_x >= dst_width || out_y < 0 || out_y >= dst_height) continue;

            rgb_dst[(out_y * dst_width + out_x) * 3 + 0] = (unsigned char)((R < 0) ? 0 : ((R > 255) ? 255 : R));
            rgb_dst[(out_y * dst_width + out_x) * 3 + 1] = (unsigned char)((G < 0) ? 0 : ((G > 255) ? 255 : G));
            rgb_dst[(out_y * dst_width + out_x) * 3 + 2] = (unsigned char)((B < 0) ? 0 : ((B > 255) ? 255 : B));
        }
    }
}
    ...
    if (src->format == IMAGE_FORMAT_RGB888) {
        ...
        if (src->format == dst->format) {
            reti = crop_and_scale_image_yuv420sp(src->virt_addr, src->width, src->height,
                ...
                dst->virt_addr, dst->width, dst->height,
                dst_box_x, dst_box_y, dst_box_w, dst_box_h);
        } else if (dst->format == IMAGE_FORMAT_RGB888) {
            int yuv_size = dst->width * dst->height * 3 / 2;
            unsigned char* yuv_tmp = (unsigned char*)malloc(yuv_size);
            if (yuv_tmp == NULL) return -1;

            reti = crop_and_scale_image_yuv420sp(src->virt_addr, src->width, src->height,
                src_box_x, src_box_y, src_box_w, src_box_h,
                yuv_tmp, dst->width, dst->height,
                dst_box_x, dst_box_y, dst_box_w, dst_box_h);
            
            if (reti == 0) {
                yuv420sp_to_rgb888(yuv_tmp, dst->width, dst->height, src->format, 
                                dst->virt_addr, dst->width, dst->height,
                                dst_box_x, dst_box_y, dst_box_w, dst_box_h);
            }
            free(yuv_tmp);
        } else {
            printf("no support conversion from %d to %d\n", src->format, dst->format);
            reti = -1;
        }
    } else {
...
static int check_rga_available() {
